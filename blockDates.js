...
async function calBlockNumber(startDatetime) {
  ...
  const firstStageDatetime = new Date(startDatetime.valueOf() + 4*24 * oneHourMS);
  const secondStageDatetime = new Date(firstStageDatetime.valueOf() + 7*24 * oneHourMS);
  const endDatetime = new Date(secondStageDatetime.valueOf() + 13*24 * oneHourMS);
...
//新加坡 Timezone
calBlockNumber(new Date('2018-3-1 00:00:00 GMT+0800'));
